...
- for 반복문을 사용하여 i 0부터 4까지 반복
  - 각 반복마다
    ...
    - scanf 함수를 사용하여 키보드로부터 실수 입력
	  - 포인터 ptrArr + i가 가리키는 주소인 arr의 각 요소에 저장
...
```
- for 반복문을 사용하여 i 0부터 4까지 반복
	- 각 반복마다 scanf 함수를 사용하여 키보드로부터 실수 입력
	  - &arr[i]에 저장
...
*(arr + i) += 2;
```
  - 각 반복마다 arr + i가 가리키는 arr[i]에 2를 더함
...
printf("%.1f ", *(arr + i));
```
  - 각 반복마다 arr + i가 가리키는 arr[i]를 소수점 첫째 자리까지 출력
...
ptrArr[i] += 2;
```
  - 각 반복마다 ptrArr[i]가 가리키는 arr[i]에 2를 더함
