...
```
- for 반복문을 사용하여 0부터 4까지 반복
  - 각 반복마다 문자열 "arr[i] 초기화: " 출력 후, scanf 함수를 사용하여 키보드로부터 실수 입력 받아
	- 포인터 ptrArr이 가리키는 배열 arr의 각 요소에 저장하고, 포인터 ptrArr를 증가
...
```
- for 반복문을 사용하여 0부터 4까지 반복
  - 각 반복마다 포인터 ptrArr이 가리키는 배열 arr의 각 요소에 2를 더하고, 포인터 ptrArr를 증가
...
- 문자열을 출력하는 printf 함수를 사용하여
  - 이스케이프 시퀀스 \n로 개행, 문자열 "float arr[5] = { " 출력 후,
  - for 반복문을 사용하여 0부터 4까지 반복
	- 각 반복마다 포인터 ptrArr이 가리키는 배열 arr의 각 요소를 소수점 첫째 자리까지 출력하고, 포인터 ptrArr를 증가
  - 마지막으로 문자열 "}\n" 출력
